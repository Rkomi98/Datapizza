...
Welcome back! We've covered clients, memory, tools—all the building blocks. Now we're putting them together into something more powerful: autonomous AI agents.

[Visual: Show evolution from chatbot to agent]

An agent isn't just a chatbot with tools. It's a system that can reason, plan, and take multiple actions to solve problems. It operates independently within the boundaries you set.

Today we're building your first agent from scratch. You'll learn how agents think, how to configure their behavior, and how to control their execution.

By the end of this video, you'll have a working agent that can solve multi-step problems autonomously.

Let's build it.

[Transition: "What Is an Agent?"]

## Content Main (7.5 min)

### Agent Architecture (2 min)

An agent is fundamentally different from a chatbot. A chatbot responds to messages. An agent pursues goals.
...
Let's recap: We built autonomous agents that reason and act to solve problems. We learned how to configure behavior through system prompts and parameters. And we explored execution modes from synchronous to streaming.

[Visual: Show agent components diagram]

Agents are powerful, but single agents have limits. In the next video, we're building multi-agent systems—teams of specialized agents working together to solve complex problems.

Before that, experiment with agent behavior. Try different system prompts, add new tools, play with max_steps. See how small changes create different reasoning patterns.
